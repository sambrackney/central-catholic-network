    ...
    <div className="min-h-screen" style={{ background: 'var(--cc-surface)', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <section className="bg-white border-y py-16" style={{ borderColor: 'var(--cc-border)' }}>
        <div className="max-w-3xl mx-auto px-6 text-center">
          ...
          <blockquote className="text-2xl md:text-3xl font-semibold leading-snug mt-5 mb-5"
            style={{ color: 'var(--cc-navy)' }}>
          ...
          <p className="text-sm leading-relaxed" style={{ color: 'var(--cc-text-muted)' }}>
            Central Connect is the directory, network, and community hub for every Central Catholic alumnus —
            bridging the generations of Vikings who came before with those still carrying the tradition forward today.
          </p>
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-6 py-20">
        <div className="text-center mb-14">
          ...
          <p className="text-sm max-w-xl mx-auto" style={{ color: 'var(--cc-text-muted)' }}>
            Built around the way Central Catholic alumni actually connect — professional, personal, and proud.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map(f => (
            <div key={f.title}
              ...
              <h3 className="text-sm font-semibold mb-2" style={{ color: 'var(--cc-navy)' }}>{f.title}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {steps.map((s, i) => (
              <div key={s.title} className="text-center">
                ...
                <h3 className="text-sm font-semibold mb-2" style={{ color: 'var(--cc-navy)' }}>{s.title}</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="text-sm leading-relaxed mb-6" style={{ color: 'var(--cc-text-muted)' }}>
              Every profile captures your complete journey — from the clubs and sports you loved at Central
              Catholic through college, career milestones, and the skills you've built along the way.
            </p>
          ...
          {/* Mock profile card */}
          <div className="bg-white rounded-2xl border shadow-xl overflow-hidden max-w-xs mx-auto w-full"
            style={{ borderColor: 'var(--cc-border)' }}>
            ...
            <div className="px-5 pb-5">
              <div className="-mt-8 mb-3">
                <div className="w-16 h-16 rounded-full border-4 border-white flex items-center justify-center text-lg font-bold text-white"
                  style={{ background: 'var(--cc-navy)' }}>SB</div>
              </div>
              ...
              <div className="mt-4 pt-4 border-t" style={{ borderColor: 'var(--cc-border)' }}>
                <p className="text-xs font-semibold mb-1.5" style={{ color: 'var(--cc-navy)' }}>Skills</p>
                <div className="flex flex-wrap gap-1">
                  {['Leadership', 'Public speaking', 'Networking'].map(s => (
                    <span key={s} className="text-[11px] px-2 py-1 rounded-full border"
                      style={{ borderColor: 'var(--cc-border)', color: 'var(--cc-text-muted)' }}>{s}</span>
                  ))}
                </div>
              </div>
            </div>
      ...
      <section className="py-24" style={{ background: 'linear-gradient(160deg, var(--cc-navy) 0%, #0f1a30 100%)' }}>
        <div className="max-w-3xl mx-auto px-6 text-center">
          <Image src="/cc-seal.png" alt="seal" width={60} height={60} className="mx-auto mb-6 opacity-75" />
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
            Ready to reconnect with your<br />
            <span style={{ color: '#e8c050' }}>fellow Vikings?</span>
          </h2>
      ...
      <footer className="bg-white border-t py-8" style={{ borderColor: 'var(--cc-border)' }}>
        <div className="max-w-6xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <p className="text-xs" style={{ color: 'var(--cc-text-muted)' }}>Made with pride by Samuel Brackney '25</p>
        </div>
      </footer>
    </div>
  )
...
const features = [
  { icon: '📰', title: 'Alumni Feed', description: 'Share career milestones, internship wins, grad school news, and anything worth celebrating with the Central Catholic community.' },
  { icon: '🔍', title: 'Alumni Directory', description: 'Search every Viking by graduation year, industry, location, or current company. Find people who walked the same halls as you.' },
  { icon: '🤝', title: 'Mentorship Matching', description: 'Alumni mark themselves as open to mentoring. Students and younger grads can request guidance from those who have been there.' },
  { icon: '💼', title: 'Opportunities Board', description: 'Alumni post internships, full-time roles, networking events, and webinars — exclusively for the Central Catholic community.' },
  { icon: '📅', title: 'Events', description: 'From class reunions to networking nights and fundraisers, keep up with everything happening in the Viking community.' },
  { icon: '💬', title: 'Direct Messaging', description: 'Message any alum directly and join group chats organized by industry, graduation year, or former clubs and sports teams.' },
]

const steps = [
  { title: 'Create your profile', description: 'Sign up and fill in your Central Catholic experience — graduation year, clubs, sports, and where life has taken you since.' },
  { title: 'Find your network', description: 'Search the alumni directory, filter by what matters to you, and reconnect with classmates or find a mentor in your field.' },
  { title: 'Give back and grow', description: 'Post opportunities, share updates, host events, and help the next generation of Vikings find their path.' },
]
